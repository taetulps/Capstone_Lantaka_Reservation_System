...
const displayDayContainerMonth = document.querySelector(
    ".days-container-month"
);
...
function updateWeekHeader(weekStart) {
    ...
    if (sameMonth && sameYear) {
        weekHeader.textContent = `${weekStart.format(
            "MMMM D"
...
function renderMonth() {
    ...
    for (let i = 0; i < 35; i++) {
        ...
        daysRender += `
      <div class="date-cell-month ${dateStatus}" data-iso="${displayDay.format(
            "YYYY-MM-DD"
        )}">
...
function renderWeek() {
    ...
    for (let i = 0; i < 7; i++) {
        ...
        daysRender += `
      <div class="date-cell-week ${dateStatus}" data-iso="${displayDay.format(
            "YYYY-MM-DD"
        )}">
        <span class="day-number">${displayDay.format("D")}</span>
...
function updateStats(stats) {
    ...
    if (totalReservationsEl)
        totalReservationsEl.textContent = stats.totalReservations ?? 0;
    if (occupancyRateEl)
        occupancyRateEl.textContent = `${Number(
    ...
    if (checkOutsTodayEl)
        checkOutsTodayEl.textContent = stats.checkOutsTodayCount ?? 0;
}
